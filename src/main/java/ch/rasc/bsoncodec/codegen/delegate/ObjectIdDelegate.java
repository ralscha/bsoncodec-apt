...
public class ObjectIdDelegate implements CodeGeneratorDelegate {
	...
	public void addEncodeStatements(TypeMirror type, CodeGeneratorContext ctx) {
		ctx.builder().addStatement("writer.writeObjectId($L)", ctx.getter());
	}

	@Override
	public void addDecodeStatements(TypeMirror type, CodeGeneratorContext ctx) {
		ctx.builder().addStatement(ctx.setter("reader.readObjectId()"));
	}

	@Override
	public boolean accepts(TypeMirror type) {
		return Util.isSameType(type, ObjectId.class);
	}
